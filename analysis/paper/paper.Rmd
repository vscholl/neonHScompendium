...
This is because NEON sub-contracted the PUUM flights to the Global Airborne Observatory, which has a spectrometer with a different wavelength range compared to the NIS payloads *([cite?](https://www.neonscience.org/observatory/observatory-blog/remote-sensing-partnership-made-paradise))*. 
...
**Maybe in appendix, make a table or something to show species info for which ones were used after cleaning/filtering**
...
There were 49 species classes and 
...
Variable importance for species classification (Figure \@ref(fig:varImpSpecies))
...
Histogram showing which scientific names have at least 20 samples to be included in the classification analysis. Note that the "Larix sp." invididuals are classified at the genus level. 
...
```{r fig-confMatSpecies, fig.cap ="Confusion matrix Species", echo=FALSE,  out.width = "450px", message = FALSE, warning=FALSE}
library(here)
...
```{r fig-confMatGenus, fig.cap ="Confusion matrix Genus", echo=FALSE,  out.width = "450px", message = FALSE, warning=FALSE}
library(here)
...
```{r fig-varImpSpecies, fig.cap ="Variable Importance Species", echo=FALSE,  out.width = "450px", message = FALSE, warning=FALSE}
library(here)
knitr::include_graphics(here::here("analysis","figures", "varImp_species.pdf"))
```
