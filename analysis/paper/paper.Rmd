...
We filtered out any spectra with a starting wavelength of 347nm, which were all collected at the Pu'u Maka'ala Natural Area Reserve (PUUM) NEON site, Pacific Tropical Domain 20, Hawaii, since the interpolation would require too much unavailable data outside the available range.
We flagged "bad bands" from 1340-1445nm, 1790-1995nm, and greater than 2400nms o they could be excluded from later analysis.
These wavelength ranges feature noisy reflectance data due to high water absoprtion. 

Next, we removed samples belonging to species classes with few samples. 
We figured that a small sample size would lead to a poor representation in model training. 
From the initial collection of 146 species, 50 species had at least 20 samples. 
We randomly assigned each spectrum to the training or validation sets, using an 80/20 split. All 50 species (and therefore, all genus values) were represented in each set. 
...
**Figure - histogram of species counts? maybe in appendix, make a table or something to show species info for which ones were used after cleaning/filtering**
**Figure - cleaned data, make it legible, how to color if at all (the species/genus/variety color doesn't make sense anymore at this stage)...**
