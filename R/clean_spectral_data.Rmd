...
```{r, message=TRUE}
# dimensions of the spectra file 
message("dimensions of all_spectra.csv file: rows   cols")
dim(all_spectra)
...
```{r, warning=FALSE, fig.height=16, fig.width=14, eval=TRUE}
all_spectra %>%
...
```{r write_all_cleaned_spectra}
message("# of unique taxonID values:")
length(unique(cleaned_spectra$genusSpecies))

message("# of unique scientificName values:")
length(unique(cleaned_spectra$genus))

message("# of unique spectraID (uid + year) values:")
length(unique(cleaned_spectra$spectraID))

# save this cleaned data set to a csv file 
write_csv(x = cleaned_spectra,
...
ggplot(taxonCounts, aes(y = reorder(genusSpecies, n), x = n)) + 
  ...
  theme_bw() + 
  scale_fill_manual(values = c("#36454f", "#7E9E8F"))
```
Histogram using scientific name

```{r fig.width=9, fig.height=12, eval=FALSE, include=FALSE}
...
# save figure to file
#ggsave(filename = here::here("analysis","figures", "hist_samples_per_species.pdf"), width = 9, height = 9)
```
...
ggplot(distinct(taxons_GT, genusSpecies, .keep_all=TRUE), 
       aes(y = reorder(taxonID, n), x = n)) + 
...
write_csv(x = filtered_spectra,
          path = here::here("analysis", "data", "derived_data", "cleaned_filtered_spectra.csv"))
```
...
filtered_spectra %>%
  ggplot(aes(wavelength_nm, ifelse(mask, NA, reflectance), group = spectraID)) +
  geom_path(alpha = .2, color = "#36454f") +
  labs(x = '\n Wavelength (nm)',
       y = 'Reflectance \n') + 
  facet_wrap(~genusSpecies
...
# save figure to file
ggsave(filename = here::here("analysis","figures", "spectra_per_species.pdf"), width = 10, height = 10)
```
