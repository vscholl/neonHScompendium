...
```{r get-veg-data}
# set output directory within the analysis/data/raw_data path 
out_dir <- here::here("analysis","data","raw_data","spectral-lib")
dir.create(out_dir)
dir.create(file.path(out_dir, 'data'))

zipsByProduct(dpID = "DP1.10098.001",
              savepath = file.path(out_dir, 'data'),
              check.size = FALSE)
stackByTable(file.path(out_dir, "data","filesToStack10098"), folder = TRUE)
```

Read the mapping data and apparent individual data:

```{r read-map}
vegmap <- read_csv("data/filesToStack10098/stackedFiles/vst_mappingandtagging.csv") %>%
  filter(!is.na(stemAzimuth), !is.na(pointID))
vegind <- read_csv("data/filesToStack10098/stackedFiles/vst_apparentindividual.csv")
```

Get precise locations for tagged plants and merge the mapping and individual data:

```{r get-precise-locs, warning=FALSE}
vegmap <- getLocTOS(vegmap, "vst_mappingandtagging")
...
veg %>%
  filter(individualID == "NEON.PLA.D03.JERC.02221") %>%
  View
...
ggsave('~/Desktop/veg-freq.png', width = 4.5, height = 3)
...
dir.create('hs_data', showWarnings = FALSE)
csv_dir <- file.path("hs_data", "tabular")
...
for (i in seq_along(split_veg)) {
  ...
  if (!file.exists(outfile)) {
    ...
    # download data
    try(byTileAOP(dpID = "DP3.30006.001", site = site_code, year = year,
...
write_csv(all_spectra, "all_spectra.csv")
```

```{r plot-spectra}
all_spectra <- read_csv("all_spectra.csv")

p <- all_spectra %>%
  group_by(scientificName) %>%
  mutate(n = n()) %>%
  ggplot(aes(wavelength_nm, ifelse(mask, NA, reflectance), group = individualID)) +
  geom_path(alpha = .2) +
  xlab('Wavelength (nm)') +
  ylab('Reflectance') +
  facet_wrap(~reorder(scientificName, -n), labeller = label_wrap_gen()) +
  theme_minimal()
ggsave("spectra.png", plot = p, width = 15, height = 15)
```

Look at the sample sizes per species

```{r ss}
all_spectra %>%
  distinct(scientificName, spectraID) %>%
  count(scientificName) %>%
  ggplot(aes(x = n, y = reorder(scientificName, -n))) +
  geom_point() +
  ylab("Species") +
...
Plot repeat samples of individuals:
...
all_spectra %>%
  ...
  top_n(1) %>%
  `[`(1:56, ) %>%
  left_join(all_spectra) %>%
  mutate(reflectance = ifelse(mask, NA, reflectance)) %>%
  ggplot(aes(x = wavelength_nm, y = reflectance, color = factor(year))) +
  geom_line() +
