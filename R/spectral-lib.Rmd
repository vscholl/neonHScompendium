...
```{r get-veg-data}
# VS-NOTE: set output directory within the analysis/data/raw_data path 
...
```{r read-map}
vegmap <- read_csv(file.path(out_dir, "data", "filesToStack10098", "stackedFiles", 
                            "vst_mappingandtagging.csv")) %>%
  filter(!is.na(stemAzimuth), !is.na(pointID))
vegind <- read_csv(file.path(out_dir, "data", "filesToStack10098", "stackedFiles",
